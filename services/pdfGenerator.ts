...
export const generateJobOrderPDF = async (order: JobOrder): Promise<Uint8Array> => {
  ...
  drawBox(page1, MARGIN, y - rowH, col1W, rowH);
  ...
  async function drawContentColumn(startX: number, data: ProductSpec | undefined): Promise<number> {
    ...
    // Use a safe empty object if data is undefined (e.g. no product 2)
    const p: any = data || {};
  ...
  // Product 2 column always drawn. If undefined, it draws empty form structure.
  ...
  drawText(page2, order.remarks, MARGIN + 5, py - 5, S_TEXT, false, 'left', 1000); 
  py -= 60;
